...
export default function InflowTab({ selected, log, showWindow }: Props) {
  ...
  async function startRandom() {
    ...
    try {
      ...
      log.push(`✅ 카페 확인: ${rd.cafeName || rd.cafeId} (ID ${rd.cafeId}). 공개 글 목록 불러오는 중(비로그인)…`, "progress");
      // ★빠른유입 크롤 = 항상 비로그인(공개글 API가 실측으로 됨). 로그인 크롤은 계정 세션 문제만 유발 → userId 안 보냄. 방문도 비로그인.
      ...
      if (!all.length) { log.push("⚠️ 공개 글을 못 찾았어요. 비공개 카페면 '카테고리 순환(로그인)' 모드로 게시판을 지정해 크롤하세요.", "warn"); setBusy(null); return; }
